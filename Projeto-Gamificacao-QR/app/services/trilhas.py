...
A leitura de uma estação também usa um único RPC para reduzir viagens entre a
Vercel e o Supabase.
"""
...
class TrilhasService:
    ...
    def get_station(self, participant_id: str, code: str) -> dict:
        result = self.repo.rpc("trilhas_get_station", {"p_participant_id": participant_id, "p_code": code.strip()})
        ...
        result.pop("ok", None)
        qr = result.get("qr") or {}
        qr["station_type_label"] = STATION_LABELS.get(qr.get("station_type"), "Estação")
        result["qr"] = qr
    ...
    def validate_station(self, participant_id: str, code: str, physical_code: str | None) -> dict:
        result = self.repo.rpc(
            ...
            {"p_participant_id": participant_id, "p_code": code, "p_physical_code": physical_code},
        )
        if not isinstance(result, dict):
            raise AppError("Resposta inválida do banco de dados.", 503)
        if result.get("ok"):
        ...
        mapping = {
            ...
            "sequence_locked": ("Esta etapa está bloqueada. Finalize primeiro as etapas anteriores da trilha.", 409),
    ...
    def answer_challenge(self, participant_id: str, code: str, answer) -> dict:
        result = self.repo.rpc(
            ...
            {"p_participant_id": participant_id, "p_code": code, "p_answer": answer},
        )
        if not isinstance(result, dict):
            raise AppError("Resposta inválida do banco de dados.", 503)
        if result.get("ok"):
