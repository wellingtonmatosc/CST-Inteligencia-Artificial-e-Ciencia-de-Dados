...
async function loadRanking(){
  try{
    ...
    list.innerHTML=rows.map(r=>`<article class="individual-ranking-card"><div class="individual-rank-position" aria-label="${Number(r.position)}º lugar">${Number(r.position)}º</div><div class="individual-rank-main"><span class="eyebrow">Participante</span><h3>${esc(r.nick)}</h3><p>${esc(meritLine(r))}</p>${r.needs_final_tiebreak?'<span class="pill">empate técnico</span>':''}</div><div class="individual-rank-score"><strong>${Number(r.points||0)}</strong><span>pontos</span></div></article>`).join('');
