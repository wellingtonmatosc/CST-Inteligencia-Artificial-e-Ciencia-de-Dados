...
function renderQuestion(q,submit){
  ...
  box.innerHTML=`<div class="card question-card"><div class="question-eyebrow">Desafio</div><h1>${esc(state.qr.name)}</h1><p class="question-prompt">${esc(q.prompt)}</p>${q.accessibility?.alt_text?`<p class="muted">Descrição: ${esc(q.accessibility.alt_text)}</p>`:''}<form id="answerForm" class="choices">${options}<button type="submit">Responder</button></form></div>`;
...
function renderCompleted(points,milestones=[]){
  ...
  box.innerHTML=`<div class="card question-card"><div class="question-eyebrow">Concluído</div><h1>Boa! Atividade concluída.</h1><p>Esta atividade já foi registrada para hoje.</p><p><span class="result-points">+${points||0} pontos</span></p>${bonusText}<p><a href="/ranking">Ver ranking</a> · <a href="/">Meu perfil</a></p></div>`;
}

function renderFailed(){
  box.innerHTML='<div class="card question-card"><div class="question-eyebrow">Finalizado</div><h1>Atividade encerrada</h1><p>As três tentativas deste ponto já foram utilizadas hoje.</p><p><a href="/">Voltar ao perfil</a></p></div>';
}
...
async function submitAnswer(e){
  ...
  try{
    ...
    if(d.completed){
      showMessage(msg,'Atividade finalizada. Você poderá tentar este QR novamente em outro dia, com uma questão diferente.','error');
      state.status='failed';submitting=false;renderFailed();return;
    }
