...
const A11Y_DEFAULTS={fontScale:1,contrast:false,reducedMotion:false,roomy:false,reading:false};

function loadA11y(){
  try{return {...A11Y_DEFAULTS,...JSON.parse(localStorage.getItem(A11Y_STORAGE_KEY)||'{}')}}catch(_){return {...A11Y_DEFAULTS}}
}

function saveA11y(prefs){
...
function initA11yPanel(){
  ...
  panel.innerHTML=`
    <div class="a11y-panel-head">
      <div><span class="eyebrow">Preferências</span><h2 id="a11yTitle">Acessibilidade</h2></div>
      <button type="button" class="secondary compact" id="a11yClose" aria-label="Fechar painel de acessibilidade">Fechar</button>
    </div>
    <p class="muted">As preferências ficam salvas somente neste navegador.</p>
    <div class="a11y-control">
      <span><strong>Tamanho do texto</strong><small id="a11yFontStatus" aria-live="polite"></small></span>
      <div class="inline-actions"><button type="button" class="secondary compact" data-a11y="font-down" aria-label="Diminuir texto">A−</button><button type="button" class="secondary compact" data-a11y="font-up" aria-label="Aumentar texto">A+</button></div>
    </div>
  ...
  const sync=()=>{
    panel.querySelectorAll('[data-a11y-check]').forEach(input=>{input.checked=Boolean(prefs[input.dataset.a11yCheck])});
    fontStatus();
    applyA11y(prefs);
    saveA11y(prefs);
  };
