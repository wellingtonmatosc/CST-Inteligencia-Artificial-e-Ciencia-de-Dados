...
from app.api.deps import current_participant, get_participant_service, get_trilhas_service
...
def _set_cookie(response: Response, token: str, settings: Settings):
    response.set_cookie(key=settings.participant_cookie_name,value=token,max_age=settings.participant_session_days*86400,httponly=True,secure=settings.session_cookie_secure,samesite="lax",path="/")
...
@router.get("/me")
def me(participant=Depends(current_participant),game: TrilhasService=Depends(get_trilhas_service)):
    return {"participant":{"id":participant["id"],"nick":participant["nick"],"full_name":participant["full_name"],"participant_type":participant["participant_type"],"has_pin":bool(participant.get("has_password")),"is_organizer":bool(participant.get("is_organizer"))},"summary":game.participant_summary(participant["id"])}
