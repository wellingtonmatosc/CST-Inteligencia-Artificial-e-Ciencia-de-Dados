...
def _recent_activity(repo: SupabaseRepository) -> list[dict]:
    rows = (
        ...
        .select("id,participant_id,qr_point_id,validated_at,activity_date,status,station_points,challenge_points,participants(nick,full_name),qr_points(code,name)")
...
def _audit(repo: SupabaseRepository, admin: dict, action: str, participant_id: str, **metadata) -> None:
    repo.insert("audit_log", {"actor_username": admin["username"], "actor_role": admin["role"], "action": action, "entity_type": "participant", "entity_id": participant_id, "metadata": metadata})
...
def event_control(admin: dict = Depends(current_admin), repo: SupabaseRepository = Depends(get_repo)):
    require_admin_role(admin, "admin")
    return {"control": _event_control(repo), "state": repo.rpc("trilhas_event_state", {}) or {}, "readiness": repo.rpc("trilhas_event_readiness", {}) or {}}


@router.put("/event-control/testing")
def set_test_mode(payload: TestModePayload, admin: dict = Depends(current_admin), repo: SupabaseRepository = Depends(get_repo)):
    require_admin_role(admin, "admin")
    if payload.test_day > payload.duration_days:
        raise AppError("O dia de teste não pode ser maior que a duração escolhida.", 422)
    result = repo.rpc("trilhas_admin_set_test_mode", {"p_duration_days": payload.duration_days, "p_test_day": payload.test_day, "p_actor": admin["username"]})
...
def clear_test_data(payload: ConfirmationPayload, admin: dict = Depends(current_admin), repo: SupabaseRepository = Depends(get_repo)):
    require_admin_role(admin, "admin")
...
def start_official_event(payload: OfficialStartPayload, admin: dict = Depends(current_admin), repo: SupabaseRepository = Depends(get_repo)):
    require_admin_role(admin, "admin")
    if payload.confirmation.strip().upper() != "INICIAR":
        raise AppError('Digite "INICIAR" para confirmar o início oficial.', 422)
    result = repo.rpc("trilhas_admin_start_official_event", {"p_duration_days": payload.duration_days, "p_actor": admin["username"]})
...
def end_official_event(payload: ConfirmationPayload, admin: dict = Depends(current_admin), repo: SupabaseRepository = Depends(get_repo)):
    require_admin_role(admin, "admin")
...
def analytics(admin: dict = Depends(current_admin), repo: SupabaseRepository = Depends(get_repo)):
    require_admin_role(admin, "admin")
    result = repo.rpc("trilhas_admin_analytics", {}) or {}
    if not isinstance(result, dict):
        raise AppError("Não foi possível carregar as análises do evento.", 503)
    return result


@router.get("/dashboard-data")
def dashboard_data(admin: dict = Depends(current_admin), repo: SupabaseRepository = Depends(get_repo)):
    require_admin_role(admin, "admin")
    participants = (repo.raw_table("participants").select("id,full_name,nick,participant_type,campus,course_name,active,is_organizer,created_at").order("full_name").limit(1000).execute().data or [])
    ...
    station_rows = repo.raw_table("qr_points").select("id,active,validation_code_hash").execute().data or []
    active_questions = _count_where(repo, "questions", active=True)
    total_questions = _count_where(repo, "questions")
    analytics_summary = repo.rpc("trilhas_admin_analytics", {}) or {}
    summary = analytics_summary.get("summary", {}) if isinstance(analytics_summary, dict) else {}
    return {
        "mode": "individual",
        "admin_mode": "single",
        ...
        "monitoring": {"timezone": "America/Cuiaba", "unusual_window": "00:00–05:59", "unusual_recent": unusual_recent, "policy": "signal_only"},
        "ranking_rules": {"order": ["points", "stations_validated", "first_try_correct"], "exact_ties_share_position": True, "speed_used": False},
        "stats": {
            "participants_total": len(participants),
            "participants_active": sum(1 for p in participants if p.get("active")),
            "competitors_active": len(competitors),
            "participants_with_activity": int(summary.get("participants_with_activity") or 0),
            "points_total": sum(int(row.get("points") or 0) for row in ranking),
            "active_stations": sum(1 for row in station_rows if row.get("active")),
            "qrs_with_physical_code": sum(1 for row in station_rows if row.get("validation_code_hash")),
...
def set_participant_active(participant_id: str, payload: ParticipantActivePayload, admin: dict = Depends(current_admin), repo: SupabaseRepository = Depends(get_repo)):
    require_admin_role(admin, "admin")
